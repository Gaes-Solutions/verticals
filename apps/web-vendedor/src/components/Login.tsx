...
const SLUG_KEY = "gaespos_vendedor_slug";
...
function tenantDeSubdominio(): string | null {
  ...
  const sub = parts[0];
  const reservados = ["www", "admin", "app", "pos", "vendedor"];
...
export function Login({ onLogin }: { onLogin: (nombre: string) => void }) {
  ...
  return (
    <div className="flex h-full items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-lg">
        <h1 className="mb-1 font-bold text-2xl text-brand">GaesSoft Vendedor</h1>
        <p className="mb-6 text-slate-500 text-sm">Tu ruta, tus clientes y tus comisiones</p>
        ...
        {paso === "codes" && pendiente && (
          <div>
            <p className="mb-2 font-semibold text-slate-800">Códigos de respaldo</p>
            <p className="mb-3 text-slate-500 text-sm">
              Guárdalos en un lugar seguro: cada uno entra UNA vez si pierdes tu teléfono.
            </p>
            <div className="mb-4 grid grid-cols-2 gap-2 rounded-lg bg-slate-50 p-3 font-mono text-slate-700 text-sm">
              {backupCodes.map((c) => (
                <span key={c}>{c}</span>
              ))}
            </div>
            <button type="button" onClick={() => entrar(pendiente)} className={btnCls}>
