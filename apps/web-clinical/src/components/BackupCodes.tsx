...
export function BackupCodes({ codes }: { codes: string[] }) {
  ...
  function descargar() {
    const blob = new Blob([`Códigos de respaldo GaesSoft\n\n${texto}\n`], { type: "text/plain" });
  ...
  return (
    <div>
      ...
      <p className="mb-3 text-slate-500 text-sm">
        Guárdalos: cada uno sirve <strong>una sola vez</strong> si pierdes tu teléfono.{" "}
        <strong>No se vuelven a mostrar.</strong>
      </p>
      <div className="mb-3 grid grid-cols-2 gap-2 rounded-lg bg-slate-50 p-4 text-center font-mono text-slate-800 text-sm">
        {codes.map((c) => (
          <span key={c}>{c}</span>
        ))}
      </div>
