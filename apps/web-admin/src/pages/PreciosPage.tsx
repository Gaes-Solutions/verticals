...
export function PreciosPage() {
  ...
  const puedeModificar = puede("precios.modificar");
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Error al cargar las listas de precios");
    } finally {
      setCargando(false);
    }
  }, []);
    ...
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-[300px_1fr]">
        ...
        <div className="space-y-2">
          {cargando && (
            <p className="rounded-lg bg-white p-4 text-sm text-slate-400 shadow-sm">Cargando…</p>
          )}
...
function DetalleLista({ lista, onCambio }: { lista: ListaDetalle; onCambio: () => void }) {
  ...
  async function quitar(varianteId: string) {
    setError(null);
    try {
      await api(`/t/precios/listas/${lista.id}/items/${varianteId}`, { method: "DELETE" });
      onCambio();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "No se pudo quitar el precio");
    }
  }
      ...
      <div className="mb-4 rounded-lg bg-slate-50 p-3">
        ...
        {sel ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            />
            <button type="button" onClick={agregar} className="gx-btn-primary">
              Guardar
            </button>
            <button type="button" onClick={() => setSel(null)} className="gx-btn-ghost">
              Cancelar
            </button>
          </div>
        ) : (
                ...
                {resultados.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className="block min-h-10 w-full rounded-lg px-3 py-2 text-left text-sm text-slate-700 hover:bg-white"
                    >
            ...
            <tbody>
              {lista.items.map((it) => (
                <tr key={it.varianteId}>
                  <td className="gx-td">{it.variante.sku}</td>
                  <td className="gx-td text-right font-medium">
