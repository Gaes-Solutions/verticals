...
API_PORT=3000
declare -A APPS=([superadmin]=5176 [admin]=5174 [pos]=5173 [tienda]=3001 [vendedor]=5179 [partner]=5180 [clinical]=5177 [marketplace]=5178)
...
cmd_up() {
  ...
  launch "VITE_API_URL=http://localhost:$API_PORT pnpm --filter @gaespos/web-partner dev" /tmp/gaes-partner.log
  launch "VITE_API_URL=http://localhost:$API_PORT pnpm --filter @gaespos/web-clinical dev" /tmp/gaes-clinical.log
  launch "VITE_API_URL=http://localhost:$API_PORT pnpm --filter @gaespos/web-marketplace dev" /tmp/gaes-marketplace.log
  ...
  for svc in superadmin admin pos tienda clinical; do
