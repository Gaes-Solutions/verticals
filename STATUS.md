...
- **🪪 Marketplace: onboarding self-service (médico + admin) (2026-07-01)** — rama `autonomo/ventas-veterinaria`, commits `7016c1c`+: cierra el último tramo para que un médico real se liste **sin sembrar datos a mano**. **web-clinical** "Mi perfil público": el médico crea/edita su perfil (tipo, cédula, especialidades, bio, telemedicina), agrega consultorios, lo **envía a revisión** y responde reseñas (crea la ficha `Medico` si falta). **web-superadmin** "Marketplace": el admin GaesSoft lista perfiles `en_revision`, los **valida/publica** (cédula SSA) y modera reseñas escaladas. **Verificación e2e ✅**: como médico creé el perfil → borrador → "Enviar a revisión" → *En revisión*; como superadmin el panel lo listó ("Dra. Vet Demo · cédula VET-556677") → **Validar y publicar** → perfil `publicado` + en índice → aparece en la búsqueda pública del marketplace. Builds verdes. (El clic final de validar se cerró por el mismo endpoint del botón — la extensión de Chrome se desconectó en ese instante.)
- **🩺 Marketplace de citas COMPLETO — portal público "GaesSalud" + OTP + telemedicina (2026-07-01)** — rama `autonomo/ventas-veterinaria`, commits `9be327e`+telemedicina: cierra el marketplace de cara al paciente (reemplazo de Doctoralia, **sin usar ese nombre**). **App web-marketplace** (`apps/web-marketplace`, Vite+React, marca **GaesSalud**, puerto 5178): búsqueda de profesionales (texto/ciudad/telemedicina), perfil por slug con reseñas, identificación con **OTP real de 6 dígitos** (email mock + `otpDev` fuera de prod), reserva de cita, "Mis reservas", dejar reseña. **OTP real** en backend (`PacienteMaster.otpCodigo/otpExpiraAt` + migración). **Telemedicina mock-first** (plugin `telemedicine`, Daily.co con `DAILY_API_KEY` o mock): al confirmar una cita de video se crea la sala y el link aparece para paciente y médico. **Suite api 757 verde.** **Verificación e2e ✅**: como paciente (GaesSalud) busqué a Dra. Vet Demo → OTP 718032 → reservé presencial y telemedicina → "Mis reservas"; confirmé la de video → apareció **"Entrar a la videollamada"** (`meet.gaessalud.mx/room/…`) y la Cita en la agenda del tenant. Loop completo: paciente reserva en el portal → médico confirma en web-clinical → Cita real en la misma agenda. Registrado en `scripts/dev.sh` (marketplace=5178).
- **📅 Doctoralia: reserva desde el portal + confirmación (2026-07-01)** — rama `autonomo/ventas-veterinaria`, commit `16eef0b`: cierra el flujo de "confirmaciones" del marketplace (faltaba el modelo de booking; las reseñas ya referenciaban `bookingId`). **Backend**: modelo `PublicBooking` (master, enums BookingStatus/Modalidad) + migración; snapshot de contacto del paciente cross-tenant + `citaIdLocal`. Público: `POST /doctoralia/profesionales/:id/reservar` (paciente verificado crea reserva pendiente) + `GET /doctoralia/pacientes/:id/reservas`. Tenant (`/t/doctoralia`): `GET /reservas`, `POST /reservas/:id/confirmar` (crea/reusa un **Paciente local** del snapshot + genera una **Cita en la MISMA agenda compartida** y liga `citaIdLocal`) y `/rechazar`, gated por citas.leer/gestionar. **Reusa** `nextCitaFolio` y `nextNumeroExpediente` (sin duplicar). 6 tests nuevos; **suite api 756 verde**. **UI** (web-clinical): sección "Reservas Doctoralia" (pendientes/confirmadas/todas, Confirmar y agendar / Rechazar). **Verificación e2e ✅** (recepción en `clinica-vet-patitas`): reserva de Sofía Ramírez desde el portal → "Confirmar y agendar" → *"Cita agendada (CT-...000005)"* → aparece en la **agenda del 3-jul** como Paciente EXP-000002, estado Confirmada; backend: booking `confirmada`+`citaIdLocal`, Cita `confirmada` con `pacienteId`. **Pendiente (opcional)**: app web-doctoralia (portal público de cara al paciente) — hoy el marketplace se consume por API; la reserva se puede probar por endpoint.
- **🧑‍⚕️ Salud: Lab/Imagen/Hospitalización aceptan humanos (2026-07-01)** — rama `autonomo/ventas-veterinaria`, commit `962f10d`: completa la variante humana en los 3 flujos que faltaban, todos migrados al `SujetoBuscador` unificado (🐾/👤). **Laboratorio** e **Imagenología**: solicitud y listado por `mascotaId|pacienteId` (backend ya validaba XOR). **Hospitalización**: el nuevo ingreso elige mascota o humano; lista y detalle muestran el sujeto correcto (helper `nombreHosp`), el cobro al alta usa el nombre del sujeto (backend ya incluía `paciente` en list/detalle). typecheck+build verdes. **Verificación e2e ✅** (médico en `clinica-vet-patitas`): solicité **Biometría hemática** para Ana López (LAB-...000003) y la **hospitalicé** en cama H-01 (HOSP-...000004) — la lista muestra 👤 Ana + 🐾 Luna juntas; backend confirma ambos con `pacienteId` (mascotaId null). Con esto **toda la app clínica es dual vet+humano** (registro, agenda, SOAP, receta, lab, imagen, hospitalización).
- **🧑‍⚕️ Salud: variante de pacientes humanos en web-clinical (2026-07-01)** — rama `autonomo/ventas-veterinaria`, commits `483d2d7` + `081ef34`: la app clínica ya era solo-mascotas aunque el backend era polimórfico (paciente XOR mascota); ahora atiende **humanos**. **Registro** (`483d2d7`): `PacienteBuscador` reutilizable + `PacientesPage` (lista/búsqueda por nombre/CURP/expediente/tel + alta con nombre/apellidos, sexo, CURP, nacimiento, tipo de sangre, contacto, alergias + detalle con contadores) + nav "Pacientes" gated por `pacientes.leer` (una clínica humana ve Pacientes, una veterinaria ve Expedientes; el gating por permisos los diferencia). **Actos clínicos** (`081ef34`): nuevo `SujetoBuscador` unificado que busca mascotas (🐾) y pacientes (👤) a la vez según permisos y devuelve un `Sujeto` polimórfico; **SOAP** y **Receta** lo usan para pasar mascotaId|pacienteId, elegir la vertical del catálogo de diagnósticos (vet/**humano** CIE-10) y filtrar previas; elimina los `MascotaBuscador` inline duplicados; `RecetaImprimible` ya mostraba ambos. typecheck+build verdes. **Verificación e2e ✅** (médico Dra. Vet Demo en `clinica-vet-patitas`): registré/vi a la paciente humana Ana López Ruiz (EXP-000001, O+, alergia Penicilina) → SOAP con dx **R51 Cefalea** (CIE-10 humano) → **firmada** (inmutable) → Receta Tempra/Paracetamol → **emitida con QR** (RX-...000003); backend confirma consulta+receta con `pacienteId` (mascotaId null). **Pendiente (opcional)**: extender Lab/Imagen/Hospitalización al selector unificado (hoy muestran paciente pero crean por mascota).
- **💉 Vet: recordatorios de próxima vacuna (2026-07-01)** — rama `autonomo/ventas-veterinaria`: extiende el motor de recordatorios para avisar al tutor cuando se acerca la **próxima dosis** de su mascota (evita vacunas vencidas). **Reusa el mismo `ConfigRecordatorios`** (ahora con `vacunasActivo`/`vacunasDiasAntes` default 7/`vacunasCanal`/`vacunasPlantilla`) + mensajería existente + scheduler. Nuevo campo `Vacunacion.recordatorioProximaEnviadoAt` + migración aplicada a todos los tenants. Motor `enviarRecordatoriosVacunas`: barre vacunaciones con `proximaAplicacionFecha` dentro de la ventana (incluye vencidas), con **lógica de supersesión** (si ya hay una aplicación posterior de la misma vacuna al mismo sujeto, esa dosis se considera cumplida y se marca sin enviar), idempotente por `recordatorioProximaEnviadoAt`, registra `Notificacion`. Endpoint `/t/recordatorios/enviar-vacunas` + el scheduler ahora corre citas **y** vacunas por tenant. **5 tests** nuevos (envío, sin contacto, no reenviar, supersesión, apagado). **UI** (Configurar agenda): segunda tarjeta "Recordatorios de vacunas" (toggle, días antes, canal, plantilla, "Enviar ahora") + "Guardar configuración" único. **Verificación e2e ✅** (recepción 2FA en `clinica-vet-patitas`): mascota Toby con próxima dosis en 3 días → "Enviar ahora" → *"Barrido: 1 enviado, 0 sin contacto, de 1 vacuna por vencer"*; backend: `Notificacion` renderizada ("A Toby le toca su vacuna Vanguard Plus 5/CV-L el sábado, 4 de julio…") ligada al tutor + marca de idempotencia; segundo barrido → 0 (no reenvía).
- **🔔 Vet: recordatorios automáticos de citas / anti-no-show (2026-06-30)** — rama `autonomo/ventas-veterinaria`, commit `3c44b93`: avisa al tutor antes de su cita con un **link público para confirmar o reagendar**. **Configurable por el dueño** (default ON + valores recomendados por el sistema: 24 h antes, WhatsApp), reusa la mensajería existente (WhatsApp/SMS/email) y el patrón pull-based del `flows-scheduler`. **Backend**: modelo `ConfigRecordatorios` (singleton) + campos en `Cita` (`recordatorioEnviadoAt`/`recordatorioCanal`/`confirmacionToken`) + migración aplicada a todos los tenants; motor `enviarRecordatoriosCitas` (barre citas en la ventana sin avisar, manda por el canal configurado, marca enviado idempotente, registra `Notificacion`); endpoints `/t/recordatorios` (config GET/PATCH + `/enviar` manual) y **página pública autocontenida** `/citas-publico/:slug/:token` (HTML servido por el API, sin depender de otra app); scheduler `recordatorios-scheduler` (env `RECORDATORIOS_SCHEDULER_ENABLED` + `PUBLIC_BASE_URL`) cableado en `server.ts`. **6 tests** nuevos; **suite api 745 verde**. **UI** (web-clinical → Configurar agenda): sección Recordatorios (toggle, horas antes, canal, plantilla con variables, "Guardar" + "Enviar ahora"), gated por `agenda.leer/gestionar`. **Verificación e2e ✅** en navegador (recepción 2FA en `clinica-vet-patitas`): cita de Pelusa (+3 h) → "Enviar ahora" → *"Barrido: 1 enviado, 0 sin contacto, de 1 cita"* → abrí el link del tutor → **Confirmar asistencia** → *"¡Confirmada! Te esperamos."*; backend: cita `estado=confirmada` + `Notificacion` con el mensaje renderizado ligada al tutor. **Pendiente (fast-follow)**: recordatorios de vacunas por vencer (reusa este mismo `ConfigRecordatorios`).
- **🐾 Vet: cobro al alta hospitalaria (opción A, 2026-06-30)** — rama `autonomo/ventas-veterinaria`: cierra el flujo "dar de alta → cobrar" reusando la MISMA caja/corte X-Z del POS, **sin duplicar lógica por vertical**. **Backend**: nuevo endpoint compartido `POST /t/ventas/:id/cobrar` (`cobrarVenta` en `ventas/service.ts`) que cobra una venta en borrador (la que genera el alta) — valida sucursal+caja con apertura activa, valida pagos (solo métodos directos: efectivo/tarjeta/transferencia/otro; fiado/monedero/b2b van por el pipeline del POS), crea `VentaPago` y marca `estado=cobrada` + `cajaId` + `cobradaAt` + `totalCobrado`/`cambioDado` → el corte la cuenta porque filtra por cajaId+cobradaAt, no por líneas. 4 tests (`tenant-cobrar-venta`: cobra+liga caja, cambio, insuficiente→400, doble-cobro→409). Commit `0c8a77b`. **Frontend** (`web-clinical` HospitalizacionPage): al dar de alta, si quedó venta con cargos, abre `CobroAltaModal` ahí mismo (selector de caja, abre turno si no hay apertura, multi-pago, faltante/cambio, POST cobrar; "cobrar después" deja el borrador para el POS). Preset **recepción** ahora incluye `cajas.leer`+`corte.consultar` (necesarios para listar caja y detectar apertura); rol del tenant demo `clinica-vet-patitas` parcheado (37→39). typecheck+build web-clinical verdes. Commit `ffee793` (pusheado). **Verificación e2e ✅** (navegador, recepción 2FA en `clinica-vet-patitas`): alta de Rocky ($600 en 2 cargos) → modal detectó caja sin turno → "Abrir caja" → split efectivo $350 + tarjeta $250 → "Cobro registrado". Backend confirmado: hospitalización `estado=alta` + cama liberada + cargos `facturadoEnVentaId` set; venta `estado=cobrada` total/cobrado 600 cambio 0 con cajaId+cobradaAt y 2 pagos; **corte X de la MISMA caja**: ventasCount=1, ventasTotal=600, efectivoEsperado=350 → el cobro clínico cae en el corte compartido del POS, sin duplicar caja.
- **🚂 Camino a producción — llaves reales + deploy Railway (2026-06-21)**: Gaby eligió **Railway** (no Hetzner), **ambas pasarelas (Conekta+Stripe)**, **las 4 integraciones** (CFDI/Resend/WhatsApp/Skydropx); tiene dominio, **no hay remote de GitHub** (repo local rama `autonomo/ventas-veterinaria`, trabajo reciente SIN commitear). **Hallazgo clave**: los clientes reales (Stripe/Conekta/Facturama/Resend/WhatsApp/Skydropx) YA están implementados de verdad (fetch a APIs, no stubs) — el switch mock→real es por env/config. **Huecos cerrados (código puro)**: (1) **bug go-live**: el pago de *links de cobro* tenía `"mock"` hardcodeado → ahora `cobros/routes.ts` resuelve la pasarela real del tenant (`resolverPasarela` lee `ConfigTiendaEcommerce.pasarelaPagoProvider`; si no hay llave → 503 PROVIDER_UNAVAILABLE, no cae a mock); (2) nuevo campo tenant `pasarelaPagoProvider` (conekta|stripe|null) + migración `20260622040317_add_pasarela_pago_provider` aplicada + selector en web-admin **Tienda online → Pasarela de pago** + en schema/routes de ecommerce-config; (3) `.env.prod.example` + `docker-compose.prod.yml` ahora pasan TODAS las llaves (Conekta/Stripe/Resend/WhatsApp/Twilio/Skydropx/Envía/VAPID) + scheduler; (4) **nginx de SPAs Railway-ready**: `deploy/nginx-spa.conf` → `.template` con `${API_UPSTREAM}` (envsubst al arrancar), 3 Dockerfiles actualizados (`ENV API_UPSTREAM=api:3000`, copia a `/etc/nginx/templates/`). **Runbook nuevo** `docs/deploy-railway.md` (5 servicios + Postgres/Redis plugins, mapeo env con `${{...}}`, migraciones one-off, DNS, webhooks, checklist go-live). 1 test nuevo cobros (503 sin llaves). **PENDIENTE (Gaby)**: crear remote GitHub + push, crear proyecto Railway + servicios, pegar llaves reales, DNS, webhooks. Commits pendientes (no commiteado aún).
- **🚀 Camino a producción — cierres rápidos 1/2 (2026-06-21)**: tras evaluación PM ("MVP avanzado/pre-beta") Gaby dijo "vamo a darle". Cerrados los 2 pendientes de código puro antes de tocar llaves reales. **(a) Consumo de monedero en POS**: el método `monedero` ya estaba en el enum de pagos pero NO descontaba saldo. Ahora `ventas/service.ts` valida (`pagoMonedero` requiere `clienteId`, no genera cambio en efectivo) y descuenta transaccionalmente vía `aplicarCargoMonedero` (crea `MonederoMovimiento` tipo `cargo` refTipo=venta, 409 si saldo insuficiente sin mutar). POS: `CobroModal` reescrito con toggle "Usar monedero" (saldo visible) y **split monedero+otro método**, `CobroResult` ahora devuelve array `pagos[]`; `Cliente.saldoMonedero` añadido a tipos web-pos. 4 tests nuevos (`tenant-venta-monedero`: pago full, sin clienteId→400, saldo insuficiente→409, split 50/50). **(b) Scheduler de automatizaciones**: `jobs/flows-scheduler.ts` (`startFlowsScheduler` + `runFlowsTodosLosTenants`) itera tenants activos del master DB y corre `runFlowsProgramados` por cada uno; gated por env `FLOWS_SCHEDULER_ENABLED` (default false) + `FLOWS_RUN_INTERVAL_MIN` (default 360min), arrancado en `server.ts` (NO en app.ts → tests no lo disparan), `setInterval().unref()` + guarda anti-solapamiento + stop en shutdown. typecheck API + build web-pos verdes. **Pendiente camino prod**: llaves reales (Conekta/Stripe/Facturama/Resend/VAPID) + deploy Hetzner/Coolify (requiere credenciales de Gaby).
- **💰 4 features de ventas (análisis experto) (2026-06-17)**: Gaby pidió "qué falta como cliente, piensa como experto en ventas" → eligió las 4. Todas con backend+UI+tests. (1) **Link de pago / cobrar por WhatsApp**: modelo `LinkPago`, módulo `tenant/cobros` (crear/listar/cancelar + público `/cobros/publico/:token` lectura+pagar vía pago provider mock), página pública `web-tienda /cobro/[token]` + route handler, web-admin `CobrosPage` (crea link, botón WhatsApp `wa.me`, copiar). Tarjeta=pago inmediato, oxxo/spei=referencia. 6 tests. (2) **Monedero + gift cards**: `Cliente.saldoMonedero`, modelos `TarjetaRegalo`+`MonederoMovimiento`, módulo `tenant/monedero` (emitir/listar/consultar/cancelar gift card; canjear-a-monedero; saldo+movimientos+abono/cargo transaccional), web-admin `MonederoPage` 2 tabs. 7 tests. (3) **Automatizaciones (flows)**: reusa `CampanaTrigger`+`evaluarTriggers` (estaban desconectados); nuevo `campanas/flows.ts` (CRUD triggers + runner pull-based `runFlowsProgramados` para cliente_nuevo/cumpleanos/win_back/recompra, respeta opt-out+tope frecuencia+destino), rutas `/t/campanas/flows*` (+run), web-admin `AutomatizacionesPage`. 6 tests. (4) **Inteligencia de inventario**: módulo read-only `tenant/inventario-insights` (velocidad venta→días-para-agotarse+sugerencia reorden 30d; estancados con valor inmovilizado; top por margen), web-admin `InventarioInsightsPage`. 1 test. `jsbarcode`/Wallet/Zap/TrendingUp/Link2 íconos. Suite api **688 verde**. Atajo dev `scripts/dev.sh` con `VITE_TIENDA_URL` para links.
- **🖨️ Impresión: etiquetas/códigos + reportes PDF (2026-06-17)**: pedido de Gaby (impresoras). **Etiquetas y códigos** (web-admin nueva sección, `EtiquetasPage` + `components/Barcode.tsx`): busca productos, genera **código de barras Code128 del SKU** (`jsbarcode`, escaneable en el POS) + **QR** opcional (`qrcode.react`), tamaños chica 40mm (impresora de etiquetas) / mediana 55mm (hoja Avery), cantidad por producto, vista previa e **Imprimir** (CSS `.etiquetas-print`, sale solo la hoja). **Reportes imprimibles**: botón **Imprimir / PDF** en `ReportesPage` (CSS `.reporte-print` + `.no-print`, muestra periodo+fecha, formato hoja carta). Verificado en Chrome end-to-end: creamos producto → publicado → comprado online ($757) + vendido en POS ($249) → **Reportes muestra ambos canales** y el Martillo etiquetable. **Estado impresión**: ticket venta (recibo 58mm vía navegador) ✅, etiquetas/códigos ✅, reportes PDF ✅; pendiente solo el ticket térmico DIRECTO ESC/POS (Print Bridge Tauri, requiere impresora física). `jsbarcode` agregado a web-admin. typecheck/lint verdes; sin cambios de backend.
- **🛠️ Atajo dev `scripts/dev.sh` + fix defensa-en-profundidad roles (2026-06-17)**: `scripts/dev.sh` (up/down/status/logs/token:super/token:tenant/setpass/code) levanta servicios con **setsid** (sobreviven entre turnos) y genera tokens resolviendo el 2FA del superadmin solo (`apps/api/scripts/dev-tokens.ts`). Demo por API verificada: superadmin crea/edita rol predefinido de Veterinaria → **vínculo vivo** propaga a la clínica al instante; retail NO lo recibe; dueño de salud no puede sacar token sin 2FA (correcto). **2 fixes de la demo**: (1) `POST/PATCH /t/roles` del dueño no validaba que los permisos fueran de su vertical → ahora `permisosFueraDeVertical` rechaza con 400 (la UI ya los ocultaba; faltaba el backend); (2) `GET /t/roles` no filtraba por `isActive` → mostraba al dueño los presets de OTRAS verticales que el sync había desactivado (ej. retail veía Médico/Enfermera) → ahora `where:{isActive:true}`. Test rbac invertido (mezclar dueño → 400). **Demo visual confirmada en Chrome** (superadmin crea rol vet en vivo; dueño retail ve solo sus roles). Suite api **668 verde** (sellado). Pendiente menor: la suite propaga plantillas de prueba a tenants reales (vínculo vivo sobre BD dev) — limpiado a mano; idealmente el afterAll debería revertir la propagación.
- **✨ Íconos Lucide en web-admin y POS (2026-06-16)**: extendido el set Lucide al panel del dueño y al POS. web-admin: NAV lateral completo (BarChart3/Package/Tags/Upload/ShoppingBag/Receipt/PackageCheck/RotateCcw/Truck/Star/MessageCircleQuestion/FileText/Users/ShieldCheck/ShoppingCart) — `icon: LucideIcon` renderizado `<n.icon/>`; Login eye (Eye/EyeOff); campana (Bell). web-pos: CorteModal (Lock/FileText). `lucide-react` agregado a ambos. **Barrido completo (2026-06-17)**: cero emojis pictográficos en web-admin y web-pos — App.tsx (Menu), PedidosPage (Settings/Store/Truck/MapPin/Tag/X), Compras/Ventas (X), Importador (Download/FileText/Settings), Seguridad (Stethoscope/Lock/LockOpen), Dashboard (ThumbsUp), PosScreen (X/Printer), Cliente/Devolucion modales (X); ✓ de toasts/botones removidos. Solo quedan ★/☆ de rating (tipográficos OK) y un ↔ en comentario. typecheck verde ambos; render IP 200.
- **✨ Tienda: íconos SVG (Lucide) en vez de emojis (2026-06-16)**: Gaby dijo que los emojis la hacían ver vieja/tipo Eleventa. Agregado `lucide-react` a web-tienda; reemplazados emojis por íconos de línea consistentes en TODO el flujo de compra: header (Search/User/ShoppingCart/Store), top bar (Truck/ShieldCheck/CreditCard), nav (Flame), filtros (SlidersHorizontal/X), home (Flame/Sparkles/TrendingUp/PackageSearch), card (Truck/ImageOff), footer (CreditCard/Store/Banknote/Lock), carrito, producto (Heart wishlist, Truck barra/calculadora envío, Check agregar, ImageOff galería), checkout (Truck/Store/CreditCard/ImageOff + BotonEntrega acepta ReactNode), gracias (Check), pago (Lock). Pendiente (secundario, no en flujo principal): emojis en páginas de cuenta/seguimiento/error (wishlist-cuenta, notificaciones, factura, seguimiento, pedidos/[folio], not-found, error). typecheck verde; render 200 en home/catálogo/gracias.
- **🎨 Rediseño tienda — "Mercado Libre moderno" (2026-06-16)**: Gaby dijo que se veía vieja y los filtros estorbaban. Elegido (de 3 mockups) el estilo ML. **Filtros movidos a panel lateral** (`components/filtros.tsx`: `PanelFiltros` sidebar sticky en desktop + drawer bottom-sheet en móvil con botón "Filtros", `BarraFiltros` con orden dropdown + chips de filtros activos removibles + conteo de resultados). **page.tsx** reestructurado: layout 2 columnas (aside filtros + grid), breadcrumb, hero más grande, secciones Ofertas/Recién llegados/Más vendidos full-width en home. **layout.tsx**: header con barra de categorías horizontal (scroll) estilo ML + chip Ofertas. Eliminado `orden-filtros.tsx`. Verificado en :3001 (categoría/búsqueda/ofertas con productos+imágenes). **TRAMPA Next**: la 1ª petición a una ruta nueva puede salir vacía por compilación on-demand; recompila y ya. typecheck verde.
- **🧪 Datos demo Globoland (retail) — 3,000 productos (2026-06-16)**: seed `packages/db/src/seed-globoland-demo.ts` (idempotente por SKU `GLB-*`, `GLB_TOTAL` configurable) llena Globoland con 20 categorías (+CategoriaPublica) + 20 marcas + 3,000 productos (variante única con precioBase, imagen picsum, ProductoImagen, InventarioSucursal con stock variado incl. ~8% sin stock, ProductoPublicado con fotosArray + ~20% en oferta + ~10% destacado). Para probar happy-path retail (catálogo→carrito→checkout mock→pedido + POS). **GOTCHA**: el BFF de la tienda (web-tienda) autentica con `TIENDA_USER_EMAIL/PASSWORD` como usuario tenant para leer el catálogo — deben ser credenciales válidas del dueño (osmar@gmial.com / Demo!2026) o el storefront sale vacío. Dueños demo uniformados a `Demo!2026`. **Tablero móvil** `tablero-pruebas/index.html` servido en :8088 (links + copiar credenciales), apps con `--host 0.0.0.0` accesibles por IP 192.168.1.105; login admin/pos acepta `?negocio=slug`.
- **🧩 Roles predefinidos gobernados por superadmin, por vertical, con vínculo vivo (2026-06-16)**: Gaby pidió que ÉL (superadmin) cree los roles por vertical, y que cada tienda vea solo los de su vertical (solo-lectura) + pueda crear los suyos limitados a su vertical. Decisión: **A (vínculo vivo)**. **Master**: modelo `RolePlantilla` (vertical|"todas", codigo, nombre, permisos[], activo) migración `add_role_plantillas`, seed inicial 15 desde presets (`seedRolePlantillas`, mapeo codigo→verticales). **Propagación** (`role-plantilla-sync.ts`): `aplicarPlantillasATenant(slug,vertical)` espeja plantillas como roles `isPreset` en el schema del tenant + desactiva presets huérfanos; `propagarPlantilla`/`propagarEliminacionPlantilla` hacen fanout a todos los tenants de la vertical al crear/editar/borrar (vínculo vivo). `createTenant`/`onboardTenant` ahora aceptan `vertical` y aplican plantillas. **Backend superadmin** `/admin/roles-plantilla` (GET verticales, GET catalogo-permisos [completo, mezclable], GET list ?vertical, POST/PATCH/DELETE solo superadmin, auditado). **Tenant** `/t/roles/catalogo-permisos` ahora **filtrado a la vertical del dueño** (solo áreas aplica=true) — el mezclado total es exclusivo del superadmin; `/t/roles/plantillas` filtrado a su vertical. **Crear cliente** ahora pide **vertical** → el tenant recibe los roles correctos. **UI**: web-superadmin nueva sección **🧩 Roles predefinidos** (tabs por vertical + editor con permisos por área mezclables + "propaga a todos"). **Tenants demo** asignados a vertical y sincronizados (retail→cajero/vendedor/almacén; vet→médico/enfermera/recepción). **Tests**: +4 admin (CRUD+gating+propagación vínculo-vivo+permiso inválido) + permissions 27. Suite api **657 verde**, builds SPAs verdes.
- **🧩 Roles por área de negocio + mezclables + plantillas (2026-06-16)** [reemplazado parcialmente por lo de arriba; el dueño quedó limitado a su vertical]: Gaby pidió crear roles organizados por vertical (cajero=tienda, enfermera=salud…) pero **poder mezclar**. Antes el catálogo de permisos se **filtraba al único vertical** del tenant (no dejaba mezclar). Ahora: `packages/permissions` expone `categoryArea`/`areaAppliesToVertical`/`listPermissionsByArea` (5 áreas: general/tienda/abarrotes/salud/despacho) + `AREA_LABEL`. `/t/roles/catalogo-permisos` devuelve `{verticalActual, areas[]}` con **TODO** el catálogo agrupado por área marcando `aplica` (las del vertical se expanden por defecto, las demás quedan colapsadas como "mezclar") — **ya no oculta otras verticales**. Nuevo `/t/roles/plantillas` (presets etiquetados por área, sin el wildcard dueño). UI web-admin `UsuariosRolesPage`: editor de rol con bloques colapsables por área (`AreaBloque`, badge "mezclar"/"N activos") + chips "empezar desde plantilla". **Tests**: +3 unit permissions (categoryArea, listPermissionsByArea no-oculta) + 3 api rbac (catálogo por área, plantillas, rol mezclando tienda+salud). Suite api **653 verde**, permissions 27, build web-admin verde.
- **🔒 2FA completo (usuarios del negocio) + códigos de respaldo (2026-06-16)**: Gaby pidió que los clientes (web-admin/web-pos) también tengan 2FA y que el superadmin entregue **códigos de respaldo**. **Política elegida**: opt-in por usuario + el dueño puede exigirlo a todo el equipo o por rol; **forzado en verticales salud** (`salud_vet`/`salud_humana`). **DB**: `AdminUser.mfaBackupCodes`, `Usuario.mfaVerifiedAt`+`mfaBackupCodes`, nuevo modelo `ConfigSeguridad` (`require2faTodos`/`require2faRoles`) — migraciones `add_admin_backup_codes` (master) + `add_user_2fa_backup_codes_security` (tenant, aplicada a todos). **Backend**: helper `lib/mfa-backup.ts` (genera 10 códigos `xxxx-xxxx`, hash argon2, consumo 1-uso); admin `/auth/mfa/activate` ahora devuelve backupCodes, `/auth/mfa/verify` acepta código de respaldo, nuevo `/auth/mfa/backup-codes/regenerate`; **tenant** (`auth-tenant`) login con gate MFA (kind `tenant_mfa`) + `/auth/tenant/mfa/{setup,activate,verify}` (login-time) + self-service `/auth/tenant/mfa/{estado,enroll,enroll/confirm,disable,backup-codes/regenerate}`; módulo `tenant/seguridad` `/t/seguridad/politica-2fa` (GET/PUT, gated `configuracion.*`). **UI**: web-superadmin Login muestra códigos al enrolar + login con respaldo (componente `BackupCodes`); web-admin Login maneja mfaRequired/mfaSetupRequired + nueva sección **🔒 Seguridad** (activar/desactivar 2FA propio, regenerar códigos, política del equipo por rol); web-pos Login con mismo flujo MFA. `qrcode.react` agregado a web-admin y web-pos. **Tests**: +10 tenant-2fa + 4 admin backup → **suite api 650 verde**. Builds de las 3 SPAs verdes. **Gotcha reusado**: 2 logins distintos (ver memoria). **Nota**: el `mfaCodeSchema` admin se amplió a max(14) para aceptar el formato de respaldo.
- **🏪 Superadmin → Crear cliente (alta de tenants sin CLI) (2026-06-16)**: cerrado el último hueco para que Gaby dé de alta a sus clientes de tienda en línea sin terminal. **Backend** `apps/api/src/modules/admin/tenants-routes.ts` (prefix `/admin/tenants`, guard `authenticateAdmin`): `GET /` lista tenants (negocio/slug/plan/status/alta), `GET /planes` planes activos, `POST /` crea cliente vía `onboardTenant` (onboarding COMPLETO: master row + schema + migraciones + defaults + **usuario dueño en el schema** listo para `/auth/tenant/login` de web-admin) — solo rol `superadmin`, auditado (`tenant.onboarded`). Devuelve credenciales (incl. password generada one-time). `onboardTenant` ahora exportado de `@gaespos/db`. **Front** `web-superadmin` nueva sección **🏪 Clientes** (`ClientesPage.tsx`): tabla + modal "Crear cliente" (slug auto-sugerido del nombre, selector de plan, dueño con password opcional) + modal de credenciales con copiar. **+4 tests** admin (403 support, lista+planes, plan inexistente 400, slug duplicado 409) → admin.test **19 verde**. **Hallazgo arquitectónico**: hay 2 logins distintos — web-admin/web-pos usan `/auth/tenant/login` (usuario del schema, RBAC), el self-serve `/auth/signup` (billing) crea solo `tenantUserAdmin` master y **NO** el usuario-dueño del schema → un signup self-serve hoy NO podría entrar a web-admin (pendiente: unificar signupPublico con onboardTenant + página pública de registro; falta decidir dónde vive esa landing). **Clientes de prueba creados**: ferreteria-lopez, abarrotes-don-chuy (growth), boutique-bella (starter), clinica-vet-patitas (scale) — todos `Cliente!2026`. **Nota**: `onboardTenant` corre `prisma migrate deploy` por `execa` (cwd packages/db); en prod la imagen api debe incluir prisma+migraciones.
- **🏁 Tienda online al 100% (2026-06-11)**: cerrado el punch-list de huecos para considerarla ecommerce completo. **Cupón real** (antes solo se guardaba el código → el cliente pagaba completo): `evaluarCupon` sobre `CuponTenant` (monto_fijo/porcentaje/envío gratis, vigencia/usos/monto mínimo), `iniciarCheckout` descuenta del total + registra uso, endpoint `/tienda/cupon` + UI con Aplicar/feedback/línea de descuento. **Paginación** del catálogo (componente que conserva filtros, page en URL). **Cuenta**: editar perfil (`PUT /cliente-portal/me`) + **cambiar contraseña** (`POST /cambiar-password`, verifica actual con argon2) → sección "Mi perfil". **Página /gracias** post-checkout (resumen + próximos pasos + links; checkout ya no cae en /seguimiento). **UX**: `not-found.tsx`/`loading.tsx`(skeleton)/`error.tsx`(reintentar) estilizados + **wishlist → carrito** (API devuelve varianteId). **+5 tests → suite ecommerce 64 verde**, build Next 30 páginas. Commits `1ecb073`/`53d8807`/`5ac9595`. Pendiente real solo: llaves sandbox (Conekta/Facturama) + deploy. **Recordatorio**: no correr `next build` con `next dev` vivo (contamina `.next`).
- **🚢 Camino a comercializar — prep sin cuentas (2026-06-11)**: checklist `docs/lanzamiento.md`. Hecho del lado código: Dockerfiles de los 5 servicios (api + 4 frontends) + `docker-compose.prod` completo + `deploy/nginx-spa.conf`; CI `main.yml` buildea+publica a GHCR (matriz) + webhook Coolify; **Sentry** cableado (no-op sin DSN); **`tenant onboard`** (alta end-to-end + usuario dueño); **web-admin → Facturación** (config Facturama sandbox/prod sin DB) + guía `docs/sandbox-pagos-cfdi.md`. Pendiente 🔑: servidor+dominio, backups B2, llaves reales Conekta/Facturama/Resend/VAPID.

- **🎨 Tienda ML — Tanda 6 + REDISEÑO VISUAL (2026-06-11)**: Gaby dijo "lo veo muy básico" → pase de diseño + experiencia de compra. **Rediseño visual** (`13dd714`): layout con barra de confianza, **header sticky** (buscador prominente + carrito con contador + cuenta), **hero con gradiente**, `ProductoCard` elevada (hover lift, sombras, gradiente placeholder, badges %OFF/envío gratis/urgencia), **footer estructurado** (Comprar/Ayuda/Pago seguro) + **páginas `/politicas/[slug]`** (editables en admin TiendaPage, `politicasHtml` en config-publica). **Experiencia de compra**: carrito real (editar cantidad +/−, miniaturas, barra "te faltan $X para envío gratis") + repaso visual en checkout (`760ad3d`); página de producto pulida (breadcrumbs + JSON-LD BreadcrumbList, tarjetas, **calculadora "¿Cuándo llega?" por CP+estado**, desglose de reseñas por estrella) (`ed9d3c8`); **avísame cuando haya stock** (modelo `StockAlert` + endpoint `/tienda/avisos-stock` + hook en ajuste de inventario que notifica email+push al reabastecer; plantilla email `stock_disponible`; migración `add_stock_alerts`) + **vistos recientemente** (localStorage) (`67d0984`). **+6 tests** → suite ecommerce **59 verde**. Verificado en Chrome (home con hero/secciones, producto con calculadora dando "$99 · 3 días", footer). **TRAMPA CONFIRMADA**: `next build` con `next dev` vivo contamina `.next` → `a[d] is not a function` / página sin CSS; arreglo: `rm -rf .next` + reiniciar dev. Para validar build, **detener dev primero**.
- **🛍️ Tienda ML — Tanda 5: pulido conversión estilo Mercado Libre (2026-06-11)**: las señales de venta que faltaban. **Backend catálogo enriquecido** (`carrito/catalogo-service.ts`): `/tienda/catalogo` y `/catalogo/:slug` devuelven `precioDesde`, `precioPromocion` (con vigencia), `enOferta`, `descuentoPct`, `stockPublico` (suma inventario − reservado − buffer, respeta `mostrarInventarioPublico`), `stockBajo`, `envioGratis` + **productos relacionados** (`relacionadosIds` o misma categoría). Soporta **orden** (precio_asc/desc, novedad, populares) + filtros (precioMin/Max, soloOfertas, soloDisponibles, recienLlegados); orden/filtro de precio en memoria por precio efectivo, resto en BD. `config-publica` expone `envioGratisDesde`. **Storefront**: `ProductoCard` reusable con **precio tachado + badge %OFF + "últimas X piezas" + envío gratis**; página de producto con promo, urgencia, botones deshabilitados sin stock, **"También te puede interesar"** (relacionados), JSON-LD con precio de oferta + availability; **home con secciones** (Ofertas / Recién llegados / Populares) + **barra de orden/filtros** (precio, ofertas, en stock). **Direcciones guardadas** (`cliente-portal/direcciones` CRUD, única default): gestor en la cuenta + selector en checkout (autocompleta default) con campos calle/número/colonia reales + "guardar esta dirección". **Fotos en reseñas**: el cliente adjunta hasta 3 fotos comprimidas en el navegador a data URL (~1024px JPEG q0.7, no hay object storage aún), se muestran en el producto; con fotos la reseña pasa a moderación. **+6 tests** (catálogo promo/stock/orden/filtro precio, direcciones CRUD + 401). Suite ecommerce **57 verde**. Verificado en Chrome (promo −33%, relacionados, home secciones). **Pendiente**: mover fotos de reseña a object storage en prod; sección "Ofertas" usa caché 120s. **OJO**: no correr `next build` mientras `next dev` corre (contamina `.next`, da `a[d] is not a function`; arreglo: `rm -rf .next` + reiniciar dev).
- **🚚 Tienda ML — Tanda 4: auto-guías de paquetería + PWA con push (2026-06-11)**: logística automática estilo Mercado Libre/Shopify, **configurable por tenant**. **Nuevo paquete `@gaespos/paqueterias`** (patrón provider igual que `@gaespos/pagos`): interface `ShippingProvider` (`cotizar`/`crearGuia`/`parseWebhook`/`cancelarGuia`) + **Skydropx** + **Envía.com** (agregadores MX) + **mock** determinista (funciona sin API keys), 21 tests. **API**: plugin `shippingProviderFactory`; `generarGuiaPedido` (cotiza→elige tarifa→crea guía+etiqueta PDF, persiste `EnvioPedido`+tracking en el pedido, evento visible al cliente), `cancelarGuiaPedido`, `procesarWebhookEnvio` (recolectada→enviado, entregado→entregado; notifica cliente + push), `intentarAutoGuia` tras pago confirmado, cotización en vivo. Endpoints `POST /t/envios/:pedidoId/guia[/cancelar]`, `POST /t/envios/webhook`, `GET /t/envios/cotizar-vivo`. **Web Push (VAPID)**: módulo `push/service.ts` (best-effort, poda suscripciones muertas), cliente-portal `push/{subscribe,unsubscribe}`, push en `pago_confirmado`/`enviado`/`entregado` según config. **Tienda PWA**: `manifest.webmanifest` + service worker (instalable, offline shell, push + click), componente `PwaPush` en la cuenta (activar/desactivar), `config-publica` expone push+VAPID. **Admin**: TiendaPage sección "Envíos automáticos y notificaciones" (selector paquetería, auto-guía, tarifas en vivo, peso default, push + eventos); PedidosPage sección de guía (generar / descargar etiqueta / cancelar), gated por `ecommerce.pedidos_gestionar`. Flags nuevos en `ConfigTiendaEcommerce` (`paqueteriaProvider`, `paqueteriaAutoGuia`, `tarifasEnVivo`, `paqueteriaPesoDefaultKg`, `pushHabilitado`, `pushEventos`) + campos en `EnvioPedido` (`proveedorLogistico`/`carrierReal`/`guiaProveedorId`/`trackingUrl`) + tabla `PushSubscription` + enum `otro`. Migración `add_tanda4_paqueteria_push`. **+5 tests** (guía manual, 409, webhook recolectada→enviado→entregado, tracking desconocido, auto-guía). Suite ecommerce **51 verde** + paqueterías 21. Verificado en Chrome (guía `fedex · MOCK…` + etiqueta + timeline). **Pendiente**: conectar llaves sandbox Skydropx/Envía + VAPID reales; selector de tarifas-en-vivo en el checkout del storefront (hoy informativo via endpoint).
- **🛒 Tienda ML — Tanda 3: cancelar + factura + Q&A (2026-06-11)**: 3 features configurables por tenant (flags en `ConfigTiendaEcommerce`: `cancelacionCliente`, `facturacionSelfService`, `preguntasPublicas`, default ON). (1) **Cancelar compra desde el cliente** (`POST /cliente-portal/pedidos/:folio/cancelar`): solo antes de envío; si estaba pagado repone stock + reembolsa reusando `procesarDevolucion`. Botón en `/cuenta/pedidos/[folio]`. (2) **Factura CFDI self-service** (`POST/GET /cliente-portal/pedidos/:folio/factura` + `/factura/pdf`): el cliente captura sus datos fiscales → reusa `emitirCfdi` → liga `facturaCfdiId` → descarga PDF. (3) **Q&A público** (modelo `PreguntaProducto`): el cliente pregunta, la tienda modera/responde en web-admin → **Preguntas** (perm `ecommerce.resenas_moderar`); las respondidas salen en el producto. Migración `add_tanda3_cancelacion_factura_qa`. **+4 tests** (cancelar ok/409, Q&A end-to-end, gating 403). Suite ecommerce 46 verde.
- **💳 MSI real con Conekta (2026-06-11)**: cobro de tarjeta con **meses sin intereses** vía Conekta. Backend: `ConektaClient.crearIntent` ahora soporta cargo con **tarjeta** usando el **token de Conekta.js** (PCI: la tarjeta nunca toca el backend) + `monthly_installments` para MSI; `CrearIntentInput` con `cardTokenId`/`mesesSinIntereses`; checkout schema/service/routes los pasan de punta a punta. Frontend: componente `PagoTarjetaConekta` (carga Conekta.js, tokeniza la tarjeta, selector de MSI según la config del tenant) — se muestra si hay `NEXT_PUBLIC_CONEKTA_PUBLIC_KEY`, si no cae al flujo demo (mock). `/api/checkout` usa proveedor `conekta` cuando hay token (sin confirmar-mock; el webhook `order.paid` genera la venta). Env documentadas. **+2 tests Conekta (card + monthly_installments)**, pagos 27 + ecommerce 42 verde. Pendiente: probar live con llaves sandbox de Conekta.
- **🛒 Tienda ML — Tanda 1 + MSI configurables (2026-06-11)**: features del storefront **configurables por el tenant** (regla default-personalizable). Modelo `ConfigTiendaEcommerce` extendido con flags + endpoint público `GET /t/tienda/config-publica` que el BFF lee. **Panel admin** (web-admin → Tienda online → "Funciones de la tienda"): el dueño activa **MSI** (sí/no + plazos 3/6/9/12/18/24 + monto mínimo), galería con zoom, rating visible, cupón en checkout, "Comprar ahora". **Storefront** respeta cada flag: página de producto con **galería** (miniaturas + zoom/lightbox), **selector de variantes** (talla/color, precio dinámico), **rating promedio**, **badge MSI** ("hasta N meses de $X"), botón **Comprar ahora**; **checkout** con **campo de cupón** (passthrough a `cuponCodigo` → motor de promos) y **desglose MSI** ("N pagos de $X"). Migración `add_tienda_funciones_config`. Pendiente: cobro real MSI con Conekta (hoy es display+passthrough), paqueterías automáticas, Q&A público, PWA. 42 tests ecommerce verde.

- **🛡️ Superadmin de plataforma CERRADO (2026-06-10)**: Hito 6 fase 2 (panel admin GaesSoft). **Backend** (4 módulos: `/admin/metrics`, `/admin/billing-ops`, `/admin/audit`, `/admin/team`) + **auth admin con 2FA TOTP obligatorio** (login→setup/QR→activate / verify, refresh rotativo, `authenticateAdmin`) + **RBAC 3 roles** (superadmin/support/billing, `requireSuperadmin` en equipo) + **audit log inmutable** (login, mfa_enrolled, team.*, billing.invoice_void) en master DB. **Frontend nuevo `apps/web-superadmin`** (Vite+React, acento índigo, puerto 5176): login MFA con QR (`qrcode.react`) + 7 pantallas (Dashboard MRR/churn/alertas, Uso en vivo cross-tenant, Cobranza, Facturas con write-off, Suscripciones, Auditoría, Equipo CRUD + reset password/2FA), responsive + design system `gx-*`. Fix otplib v13→v12 (su código usa la API `authenticator`). **15 tests admin** (flujo MFA completo + métricas + cobranza + audit + equipo + gating support→403). Tareas #124 y #125 cerradas.
